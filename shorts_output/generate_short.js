...
// ==========================================
// SECTION 3: VOICE PROFILE & KOKORO-82M CONFIGURATION
// Configures local open-source speech model parameters.
// ==========================================
const SELECTED_VOICE_PROFILE = 'OPEN_SOURCE_SARAH'; 
...
// Normalizes text strings and triggers local Python Kokoro-82M model inference.
// ==========================================
...
function synthesizeVoiceover(rawText, outputPath, tempTag, segmentType = 'body') {
    ...
    const profile = VOICE_PROFILES[SELECTED_VOICE_PROFILE] || VOICE_PROFILES.OPEN_SOURCE_SARAH;

    // Inline Python script using Kokoro-82M local pipeline
    const pythonScript = `
import soundfile as sf
from kokoro import KPipeline
import torch

pipeline = KPipeline(lang_code='${profile.langCode}')
text = open(r"${tempTxtPath}", "r", encoding="utf-8").read()
generator = pipeline(text, voice='${profile.voiceName}', speed=${profile.speed})
...
if audios:
    ...
    sf.write(r"${outputPath}", full_audio, 24000)
`;

    const pyScriptPath = path.join(__dirname, `temp_kokoro_${tempTag}.py`);
...
async function buildShortFromTemplate(templatePath) {
    ...
    console.log(`🎙️ Active Voice: Kokoro-82M [${activeProfile.voiceName}] | Language: ${templateLang.toUpperCase()}`);
    ...
    if (hasIntroBg) {
        ...
        const introAudioPath = path.join(__dirname, `temp_slide_audio_intro_${templateFileName}.wav`);
        
        const createSilenceCmd = `"${ffmpegInstaller}" -f lavfi -i anullsrc=r=44100:cl=stereo -t 1.0 -c:a pcm_s16le -y "${introAudioPath}"`;
    ...
    for (let i = 0; i < newsSlides.length; i++) {
        ...
        const slideAudioPath = path.join(__dirname, `temp_slide_audio_${templateFileName}_${i}.wav`);
    ...
    if (hasClosingBg) {
        ...
        const closingAudioPath = path.join(__dirname, `temp_slide_audio_closing_${templateFileName}.wav`);
